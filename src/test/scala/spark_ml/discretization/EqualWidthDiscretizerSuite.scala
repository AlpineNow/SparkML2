...
class EqualWidthDiscretizerSuite extends FunSuite with LocalSparkContext {
  test("Test the equi-width discretizer 1") {
    ...
    assert(bins(0).getMissingValueBinIdx === -1)
    ...
    assert(bins(1).getMissingValueBinIdx === -1)
    ...
    assert(bins(2).getMissingValueBinIdx === -1)
  ...
  test("Test the equi-width discretizer 2") {
    val rawData = TestDataGenerator.labeledData5
    val testDataRDD = sc.parallelize(rawData, 3).cache()
    ...
    try {
      EqualWidthDiscretizer.discretizeFeatures(
        ...
        Set[Int](0),
        labelIsCategorical = true,
        Map[String, String](StringConstants.NumBins_Numeric -> "5", StringConstants.MaxCardinality_Categoric -> "5")
      )
    } catch {
      case e: CardinalityOverLimitException => exceptionThrown = true
    ...
    assert(bins(0).isInstanceOf[CategoricalBins])
    assert(bins(0).getCardinality === 5)
    assert(bins(0).getMissingValueBinIdx === 5)
    assert(bins(0).findBinIdx(0) === 0)
    assert(bins(0).findBinIdx(1) === 1)
    assert(bins(0).findBinIdx(2) === 2)
    assert(bins(0).findBinIdx(3) === 3)
    ...
    assert(bins(1).isInstanceOf[NumericBins])
    assert(bins(1).getCardinality === 4)
    assert(bins(1).getMissingValueBinIdx === 4)
    assert(bins(1).findBinIdx(0) === 0)
    assert(bins(1).findBinIdx(1) === 2)
    assert(bins(1).findBinIdx(2) === 3)
  ...
  test("Test the equi-width RDD transformation 2") {
    val rawData = TestDataGenerator.labeledData5
    val testDataRDD = sc.parallelize(rawData, 3).cache()
    ...
    while (sampleIdx < rawTxData1.length) {
      ...
      if (rawRow._2(0).isNaN) {
        assert(binId0 === bins256(0).getMissingValueBinIdx)
      } else {
        assert(rawRow._2(0).toInt === binId0)
      }

      if (rawRow._2(1).isNaN) {
        assert(binId1 === bins256(1).getMissingValueBinIdx)
      } else {
        assert(bins256(1).findBinIdx(rawRow._2(1)) === binId1)
      }
