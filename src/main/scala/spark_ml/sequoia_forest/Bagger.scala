...
object Bagger {
  ...
  def bagRDD[@specialized(Byte, Short) T](
    ...
    samplingRate: Double): RDD[(Double, Array[T], Array[Byte])] = {
    val poission = Poisson(samplingRate)
    data.map(row => {
      val counts = Array.fill[Byte](numTrees)(0)
      var treeId = 0
      while (treeId < numTrees) {
        val sampleCount = samplingType match {
          case SamplingType.SampleWithReplacement => poission.sample()
          case SamplingType.SampleWithoutReplacement => if (math.random <= samplingRate) 1 else 0
        }
  ...
  def bagArray[@specialized(Byte, Short) T](
    ...
    numTrees: Int,
    samplingType: SamplingType.SamplingType,
    samplingRate: Double): Array[(Double, Array[T], Array[Byte])] = {
    val poission = Poisson(samplingRate)
    data.map(row => {
      val counts = Array.fill[Byte](numTrees)(0)
      var treeId = 0
